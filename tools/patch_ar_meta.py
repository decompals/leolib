...
def ar_patch(ar, base_ar, new_uid, new_gid, new_filemode):
    """
    AR file headers
    ...
    i = 8
    while i < len(base_ar):
        file_name = base_ar[i:][0:][:16].decode("ASCII")
        file_size = int(base_ar[i:][48:][:10].decode("ASCII").strip())
        end = base_ar[i:][58:][:2].decode("ASCII")

        assert end == "`\n"

        if file_name.strip() == "/":
            armap_time1 = base_ar[i+16:i+16+12]
        elif file_name.strip() == "//":
            armap_time2 = base_ar[i+16:i+16+12]
...
def main():
    ...
    parser.add_argument("base", help="path to the original ar file")
    ...
    base_ar = None
    with open(args.base, "rb") as base_ar_file:
        base_ar = bytearray(base_ar_file.read())

    ar = None
    with open(args.target, "rb") as ar_file:
        ar = bytearray(ar_file.read())

    ar_patch(ar, base_ar, args.uid, args.gid, args.filemode)
